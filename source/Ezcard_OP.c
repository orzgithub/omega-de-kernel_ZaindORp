...
void IWRAM_CODE SetRompageWithHardReset(u16 page,u32 bootmode)
{
	...
	RegisterRamReset(((bootmode == 0 || bootmode == 1) ? RESET_EWRAM : 0) | RESET_PALETTE | RESET_VRAM | RESET_OAM | RESET_SIO | RESET_SOUND | RESET_OTHER);
	if(bootmode==1) {
		HardReset();
	} else if (bootmode==2 || bootmode==4) { // multiboot mode plugin, 2 for normal mb and 4 for mbz
		// TODO: for unknown reason, clear exram make the plugin unbootable so skip it untill fixed.
		/*
		int i;
